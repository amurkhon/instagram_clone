...
class User(AbstractUser, BaseModel):
    ...
    def create_verify_code(self, verification_type):
        verification_code = "".join([str(random.randint(0, 9)) for _ in range(4)])
        UserConfirmation.objects.create(user_id=self.id, verification_type=verification_type, verification_code=verification_code)
...
class UserConfirmation(BaseModel):
    ...
    verification_type = models.CharField(max_length=31, choices=TYPE_CHOICES)
